...
class iLQR:
    ...
    def get_gradients(self,F_x,F_u,x,u,V_x_next, V_xx_next):
        ...
        # TESTING: ADDING REGULARIZATION TO Q_uu ONLY
        # Additional regularization to ensure Q_uu is positive definite
        # eigs = np.linalg.eigvals(Q_uu)
        # lambda_min = np.min(np.real(eigs))
        # if lambda_min <= 0:
        #     Q_uu = Q_uu + (1.01 * abs(lambda_min)) * np.eye(Q_uu.shape[0])
